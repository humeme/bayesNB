...
# 第一个参数vocabList：createVocabList（）函数的无重复你词汇返回值，
# 第二个参数inputSet：loadDataSet () 函数的词汇表取出每一行来进行处理
def setOfWords2Vec (vocabList, inputSet):
	# 创建一个所有元素为 0 的列表，总共有六行
...
if __name__ == "__main__":
	...
	#for vec in listOPosts:
		#print(setOfWords2Vec(myVocabList, vec))
	#print (listOPosts[0])
	print (setOfWords2Vec(myVocabList,listOPosts[0]))
	#label2 = setOfWords2Vec(myVocabList,listOPosts)
